...
- AGENTS 정책 참조와 secure-storage synthetic fixture를 정리한 뒤 전체 회귀 테스트, Windows secure-storage smoke, Leerness `gate`를 성공 통과함.

## Next
- 개발 저장소 Actions secret `RELEASE_REPO_TOKEN`을 정확한 이름으로 등록한 뒤 실제 cross-repository release read/write 검증을 다시 실행.
- 이후 릴리스는 private 개발 저장소의 `.github/workflows/publish-public-release.yml`에서 공개 release 저장소로 EXE/체크섬/공개 노트만 게시.
- 모든 개발 세션은 시작 시 handoff, 완료 전 gate, 종료 시 session close 절차를 사용.

## Blockers
- 2026-09-05 실제 Actions 재검증에서도 `secrets.RELEASE_REPO_TOKEN`이 빈 값으로 전달됨. 개발 저장소의 **Settings → Secrets and variables → Actions → Repository secrets**에 정확한 이름으로 등록되어야 자동 cross-repository 배포가 활성화됨.
