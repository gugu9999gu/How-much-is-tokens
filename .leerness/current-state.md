...
## Now
- v1.0.24 OpenRouter secure multi-key profile 및 localhost request router 구현 완료, 정식 PR 검증 준비 중. <!-- leerness:auto -->
...
- key 선택은 `priority-fallback` 또는 `max-remaining`; 401/402/403/429와 network failure는 응답 commit 전 cooldown/failover하며 모든 key가 불가하면 503 fail-closed.
- upstream 응답을 client에 전달하기 시작한 뒤 stream 실패 시 다른 key로 요청을 replay하지 않음.
- 사전 Windows 검증 run `33980655618`에서 syntax, 전체 회귀 테스트, Leerness gate, DPAPI multi-profile secure-storage smoke, renderer smoke가 모두 성공함. 이후 credential header stripping/health 최소화 변경은 정식 PR CI에서 재검증 예정.

## Next
- feature branch `feat/v1.0.24-openrouter-local-router`로 정식 PR 생성 후 Windows full CI/portable EXE build 검증. <!-- leerness:auto -->
- PR 성공 시 main에 squash merge하고 v1.0.24 공개 Release를 `How-much-is-tokens-releases`에 EXE/체크섬/명시적 공개 노트만 게시.
