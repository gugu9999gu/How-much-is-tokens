...
| T-0007 | done | Claude 상태 사용량 조회 실패 재현 및 수정 | Claude 집중 테스트, 실제 로컬 캐시 조회, 관련 회귀 테스트, Electron 레이아웃 6건 및 Windows 보안 3건 통과; 전체 npm test는 변경과 무관한 기존 PowerShell UTF-8 라우터 테스트 1건만 실패 | 푸시된 브랜치에서 운영 확인 | 2026-09-21 |
| T-0008 | done | Claude 상태 사용량 조회 안정화 | 수정: lib/providers/claude.js, lib/http.js; 테스트: test/claude-usage.test.js의 12개 assert 호출; Command: node test/claude-usage.test.js; Exit: 0; 계정 캐시, 1시간 메모리 캐시, Retry-After 429 backoff 검증 | 없음 | 2026-09-21 |
