...
function isGeminiWindow(win) {
  const id = String(win && win.id || "").toLowerCase();
  const label = String(win && win.label || "").toLowerCase();
  return id.includes("gemini") || label.includes("gemini");
}

function primaryAntigravityWindow(windows) {
  const available = Array.isArray(windows) ? windows.filter((win) => win && win.remainingPct != null) : [];
  const gemini = available.filter(isGeminiWindow);
  return tightest(gemini.length ? gemini : available);
}
...
async function fetchUsage() {
  ...
  if (live.ok && live.windows.length) {
    const primary = primaryAntigravityWindow(live.windows);
    return {
      id: ID,
      name: NAME,
      brand: BRAND,
  ...
  if (context.hasSnapshot) {
    ...
    const primary = primaryAntigravityWindow(activeWindows.length ? activeWindows : context.windows);
