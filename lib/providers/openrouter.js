...
function parseKeyPayload(payload) {
  ...
  const extras = [];
  if (limit != null) extras.push({ label: "Key 한도", value: money(limit) });
  if (limitRemaining != null) extras.push({ label: "Key 잔여", value: money(Math.max(0, limitRemaining)) });
  if (usage != null) extras.push({ label: "Key 누적", value: money(usage) });
  if (usageDaily != null) extras.push({ label: "오늘", value: money(usageDaily) });
  if (usageWeekly != null) extras.push({ label: "이번 주", value: money(usageWeekly) });
  if (usageMonthly != null) extras.push({ label: "이번 달", value: money(usageMonthly) });
...
function parseCreditsPayload(payload) {
  ...
  return {
    ...
    extras: [
      { label: "잔여 크레딧", value: money(remaining) },
      { label: "총 충전", value: money(purchased) },
      { label: "계정 사용", value: money(used) },
    ],
...
async function fetchUsage(settings = {}, secrets = {}) {
  ...
  const openrouter = secrets.openrouter || {};
  const apiKey = typeof openrouter.apiKey === "string" ? openrouter.apiKey.trim() : "";
  const managementKey = typeof openrouter.managementKey === "string" ? openrouter.managementKey.trim() : "";
  ...
  // Account credit balance is the most useful global remaining amount, so show
  // it first when a management key is available. A per-key spending cap is a
  // separate quota and must never be presented as the account balance.
  if (creditInfo && creditInfo.window) windows.push(creditInfo.window);
  if (keyInfo && keyInfo.window) windows.push(keyInfo.window);
  if (creditInfo) extras.push(...creditInfo.extras);
  if (keyInfo) extras.push(...keyInfo.extras);
