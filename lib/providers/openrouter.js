...
function secureStoredKeys() {
  try {
    const providerSecrets = require("../secure-secrets").loadProviderSecrets();
    return providerSecrets.openrouter || {};
  } catch {
    return {};
  }
}
...
async function fetchUsage(settings = {}, secrets = {}) {
  ...
  const { apiKey, managementKey } = resolveKeys(secrets);
  if (!apiKey && !managementKey) {
    return {
      id: ID,
      name: NAME,
      brand: BRAND,
      status: "missing",
      hint: "설정에서 OpenRouter API Key 또는 Management Key를 저장하세요.",
    };
  }
  ...
  const windows = [];
  // Account credit balance is the global remaining amount. A per-key spending
  // cap is a separate quota and must never be presented as the account balance.
  if (creditInfo && creditInfo.window) windows.push(creditInfo.window);
  if (keyInfo && keyInfo.window) windows.push(keyInfo.window);

  // The standard renderer shows four compact extras. Prioritize the values the
  // user needs to make a spend decision; detailed key limits remain visible as
  // their own quota window above.
  const extras = [];
  if (creditInfo) extras.push({ label: "잔여 크레딧", value: money(creditInfo.remaining) });
  ...
  if (extras.length < 4 && creditInfo) extras.push({ label: "총 충전", value: money(creditInfo.totalCredits) });
  if (extras.length < 4 && keyInfo && keyInfo.usageWeekly != null) extras.push({ label: "이번 주", value: money(keyInfo.usageWeekly) });
  if (extras.length < 4 && apiKey && !keyInfo) extras.push({ label: "API Key", value: errorText(keyRes, "조회") });
  if (extras.length < 4 && managementKey && !creditInfo) extras.push({ label: "Management", value: errorText(creditsRes, "조회") });
