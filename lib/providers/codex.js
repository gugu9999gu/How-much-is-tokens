...
function creditSnapshotFromRateLimits(rateLimitsRead) {
  ...
  if (individual && typeof individual === "object") {
    ...
    let balance = limit != null && used != null ? Math.max(0, limit - used) : null;
...
async function fetchUsage(settings = {}) {
  ...
  let data = res.json;
  let rateLimitsRead = await readAccountRateLimits();
  ...
  // Opt-in only. The policy requires the app-server to classify the current
  // state as an actual `rate_limit_reached`; 0% UI rounding or stale snapshots
  // are never enough to spend a ticket. Persistent fingerprints and
  // idempotency keys prevent repeat consumption in the same limit epoch.
