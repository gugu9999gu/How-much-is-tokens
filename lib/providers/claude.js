...
function legacyWindows(data) {
  ...
  // Keep future model-scoped flat keys visible without requiring a release for
  // every Anthropic model name. Known non-model buckets are excluded.
  const known = new Set(["seven_day", "seven_day_sonnet", "seven_day_opus", "seven_day_fable", "seven_day_oauth_apps", "seven_day_cowork"]);
  for (const [key, raw] of Object.entries(data || {})) {
    if (!key.startsWith("seven_day_") || known.has(key) || !raw || typeof raw !== "object") continue;
    const modelName = raw.display_name || raw.displayName || raw.model || key.slice("seven_day_".length).replace(/_/g, " ");
    const win = windowFrom(raw, slug(key), `${modelName} 주간 한도`);
...
function sameWindow(a, b) {
  ...
  return al === bl || (al.includes("주간") && bl.includes("주간") && !al.includes("sonnet") && !al.includes("opus") && !al.includes("fable") && !bl.includes("sonnet") && !bl.includes("opus") && !bl.includes("fable"));
}
...
function extraUsageCreditBalance(extraUsage) {
  ...
  const limit = amountFromMinor(extraUsage.monthly_limit, 2);
  const used = amountFromMinor(extraUsage.used_credits, 2);
