...
async function fetchUsage() {
  ...
  const data = res.json;
  const windows = [
    windowFrom(data.five_hour, "five_hour", "5시간"),
    windowFrom(data.seven_day, "seven_day", "주간"),
    windowFrom(data.seven_day_sonnet, "sonnet", "Sonnet 주간"),
    windowFrom(data.seven_day_opus, "opus", "Opus 주간"),
  ].filter(Boolean);
