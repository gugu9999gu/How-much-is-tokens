...
function grokConfigDir(profile = null) {
  ...
  return process.env.GROK_CONFIG_DIR || path.join(home(), ".grok");
}
...
function pickAuth(profile = null) {
  const file = readJson(path.join(grokConfigDir(profile), "auth.json"));
...
async function fetchUsage(_settings = {}, _secrets = {}, profile = null) {
  ...
  if (!auth) {
    return {
      ...
      status: "missing",
      hint: profile
        ? "이 Grok 프로필에 로그인 정보가 없습니다. 해당 GROK_CONFIG_DIR에서 Grok CLI 로그인을 실행하세요."
