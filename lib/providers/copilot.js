...
function tokenFromGhCli(execImpl = execFileSync, env = process.env) {
  try {
    const token = String(execImpl("gh", ["auth", "token"], {
      encoding: "utf8",
      windowsHide: true,
      timeout: 2_000,
...
function findProfileToken(profile, options = {}) {
  if (!profile || !profile.configDir) return null;
  const env = options.env || profileEnvironment(profile);
  const hosts = path.join(profile.configDir, "hosts.yml");
  const fileToken = fs.existsSync(hosts) ? tokenFromGhHosts(hosts) : null;
  if (fileToken) return { token: fileToken, source: hosts };
  const cliToken = tokenFromGhCli(options.execFileSyncImpl || execFileSync, env);
  return cliToken ? { token: cliToken, source: "gh auth profile" } : null;
}

function findToken(settings, options = {}) {
  if (options.profile) return findProfileToken(options.profile, options);
  ...
  const ghCliToken = tokenFromGhCli(options.execFileSyncImpl || execFileSync, options.env || process.env);
  if (ghCliToken) return { token: ghCliToken, source: "gh auth" };
...
function billingFromCopilot(data) {
  if (!data || typeof data !== "object") return null;
  return billingCycle({
    renewsAt: data.quota_reset_date_utc || data.quota_reset_date,
    label: "할당 갱신",
    source: "copilot-quota-reset",
  });
}
...
async function fetchUsage(settings = {}, _secrets = {}, profile = null) {
  const creds = findToken(settings, { profile });
  if (!creds) {
    return {
      id: ID,
      name: NAME,
      ...
      status: "missing",
      hint: profile
        ? "이 GitHub CLI 프로필에 로그인 정보가 없습니다. 해당 GH_CONFIG_DIR에서 gh auth login을 실행하세요."
        : "Copilot 연결 정보가 없습니다. 설정의 Copilot 로그인 버튼 또는 VS Code/GitHub CLI 로그인을 사용하세요.",
    };
  }
  ...
  if (res.status === 401) {
    return {
      ...
      status: "login",
      hint: profile
        ? "이 GitHub CLI 프로필 인증이 거부되었습니다. 해당 프로필에서 다시 로그인하세요."
        : "GitHub 인증이 거부되었습니다. 설정에서 Copilot 로그인을 다시 실행하세요.",
    };
  }
  ...
  const data = res.json;
  const rawAccountId = data.login || data.github_login || data.user_id || data.userId || null;
  const accountKey = stableAccountKey(ID, rawAccountId);
  ...
  return {
    id: ID,
    accountKey,
    accountEmail: data.email || data.email_address || null,
    accountLogin: data.login || data.github_login || null,
    accountId: data.user_id || data.userId || data.id || rawAccountId,
    name: NAME,
    brand: BRAND,
    status: "ok",
    ...
    billing: billingFromCopilot(data),
  };
}
