...
function preferredWindowsCommand(lines) {
  const candidates = Array.isArray(lines) ? lines.filter(Boolean) : [];
  return candidates.find((value) => /\.(?:exe|com)$/i.test(value))
...
function resolveCommand(commands, options = {}) {
  ...
  for (const command of candidates) {
    ...
    if (platform === "win32") {
      const preferred = preferredWindowsCommand(resolved);
...
function launchCredentialLogin(providerId, profile = null, options = {}) {
  ...
  try {
    const child = spawnImpl(comspec, ["/d", "/s", "/c", commandLine], {
      ...
      detached: true,
      // The /c payload already contains CMD quoting for START and the target
      // executable. Node's default Windows argv escaping rewrites those quotes
      // as backslash-quote sequences, which START then treats as a bad path.
      windowsVerbatimArguments: true,
      // Login is interactive. Hiding this detached console makes the button
      // appear to do nothing even though the CLI process was launched.
      windowsHide: false,
      stdio: "ignore",
    });
