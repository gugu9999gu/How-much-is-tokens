...
function resetFingerprint(response) {
  const reached = rateLimitSnapshots(response)
    ...
    .map(({ id, value }) => ({
      ...
      primaryUsed: value.primary && value.primary.usedPercent != null ? Number(value.primary.usedPercent) : null,
      secondaryUsed: value.secondary && value.secondary.usedPercent != null ? Number(value.secondary.usedPercent) : null,
...
function selectCreditId(resetTickets) {
  ...
  const ticket = resetTickets.tickets.find((item) => item && item.status === "available" && item.id);
  return ticket ? String(ticket.id) : null;
}
...
async function maybeAutoConsumeReset(options = {}) {
  ...
  let result;
  try {
    result = await consumeImpl({ idempotencyKey, creditId });
  } catch (err) {
