...
// Third-party helper is optional and pinned to the reviewed commit. Never pull
// @latest at runtime: account switching touches OS credential stores and must
// not silently change implementation underneath an existing widget release.
const AGM_COMMIT = "1d3ce8497e36ffa60c3b4e369168315a7ae4d469";
const AGM_REPOSITORY = `github.com/shyim/agm@${AGM_COMMIT}`;
...
function parseAccountInfo(output) {
  ...
  for (const line of String(output || "").split(/\r?\n/)) {
    ...
    if (!match) continue;
    const remainingPct = Math.max(0, Math.min(100, Number(match[3])));
    windows.push({
...
function profileIdForEmail(email) {
  return `agm-${crypto.createHash("sha256").update(String(email || "").trim().toLowerCase()).digest("hex").slice(0, 12)}`;
}
...
async function listManagedUsageRows(options = {}) {
  ...
  return Promise.all(accounts.map(async (account, index) => {
    ...
    const profileId = profileIdForEmail(normalizedEmail);
    return {
      id: `antigravity:${profileId}`,
      providerId: "antigravity",
      profileId,
      externalAccountRef: account.email,
      accountKey: stableAccountKey("antigravity", normalizedEmail),
...
async function managerStatus(options = {}) {
  const executable = resolvedAgm(options);
  const go = options.goExecutable || resolveCommand(["go"], options);
  if (!executable) return { installed: false, goAvailable: !!go, repository: AGM_REPOSITORY, commit: AGM_COMMIT };
  const listed = await runFile(executable, ["list"], { ...options, timeout: 5_000 });
  return { installed: true, executable, goAvailable: !!go, ready: listed.ok, repository: AGM_REPOSITORY, commit: AGM_COMMIT, error: listed.ok ? null : (listed.stderr.trim() || listed.error) };
}
