...
function saveProviderSnapshot(provider, now = Date.now(), filePath = usageCachePath()) {
  ...
  // v1.0.20 and earlier keyed every provider only by provider.id. Once a
  // stable account key is available, remove that ambiguous legacy slot so a
  // later login cannot accidentally inherit another account's stale quota.
  if (provider.accountKey && cacheKey !== provider.id) delete store.providers[provider.id];
...
function validCachedCredits(creditBalances, now) {
...
function resolveEntry(store, providerOrId) {
  ...
  // One-time compatibility for pre-account-key snapshots. Only use it if the
  // caller has not already established a different account-key snapshot.
...
function loadProviderSnapshot(providerOrId, now = Date.now(), filePath = usageCachePath()) {
  ...
  const credits = validCachedCredits(cached.creditBalances, now);
