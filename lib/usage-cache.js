...
const LEGACY_GROK_PRODUCT_WINDOWS = new Set(["grokbuild", "grokcode", "grokchat", "grokappbuilder", "grokimagine"]);
...
function sanitizeProvider(provider) {
  return {
    ...
    providerId: safeText(provider.providerId),
    instanceKey: safeText(provider.instanceKey),
    profileId: safeText(provider.profileId),
    managedBy: safeText(provider.managedBy),
    accountKey: safeText(provider.accountKey),
    accountLabel: provider.managedBy === "agm" ? undefined : safeText(provider.accountLabel),
    accountOrder: finiteValue(provider.accountOrder) ? Number(provider.accountOrder) : undefined,
...
function readStore(filePath = usageCachePath()) {
  ...
  if (!data || data.version !== CACHE_VERSION || !data.providers || typeof data.providers !== "object") return { version: CACHE_VERSION, providers: {} };
  return data;
}
...
function saveProviderSnapshot(provider, now = Date.now(), filePath = usageCachePath()) {
  ...
  store.providers[cacheKey] = { savedAt: now, provider: sanitizeProvider(provider) };
