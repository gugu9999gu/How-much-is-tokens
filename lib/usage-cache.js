...
function hasGauge(provider) {
  ...
  if (Number.isFinite(Number(provider.remainingPct))) return true;
  return (provider.windows || []).some((win) => Number.isFinite(Number(win && win.remainingPct)));
}
...
function safeWindow(win) {
  ...
  return {
    ...
    remainingPct: win.remainingPct == null ? null : Number(win.remainingPct),
    usedPct: win.usedPct == null ? null : Number(win.usedPct),
    resetAt: win.resetAt == null ? null : Number(win.resetAt),
...
function sanitizeProvider(provider) {
  return {
    ...
    remainingPct: provider.remainingPct == null ? null : Number(provider.remainingPct),
    usedPct: provider.usedPct == null ? null : Number(provider.usedPct),
    resetAt: provider.resetAt == null ? null : Number(provider.resetAt),
...
function validCachedWindows(windows, now) {
  return (windows || []).filter((win) => {
    if (!Number.isFinite(Number(win && win.remainingPct))) return false;
...
function loadProviderSnapshot(id, now = Date.now(), filePath = usageCachePath()) {
  ...
  if (!entry || !entry.provider || !Number.isFinite(Number(entry.savedAt))) return null;
  ...
  const originalWindows = cached.windows || [];
