...
const PROVIDER_ARGS = { copilot: ["copilot"] };
const PROVIDER_BIN_ENV = {
  codex: "CODEX_BIN", claude: "CLAUDE_BIN", grok: "GROK_BIN",
  cursor: "CURSOR_BIN", copilot: "GH_BIN", antigravity: "AGY_BIN",
};

function commandOutput(command, args, env, execImpl = execFileSync) {
  try {
    return String(execImpl(command, args, { encoding: "utf8", windowsHide: true, timeout: 2_000, stdio: ["ignore", "pipe", "ignore"], env }) || "");
  } catch { return ""; }
}
...
function switchAntigravityProfile(profile, options = {}) {
  ...
  try {
    (options.execFileSyncImpl || execFileSync)(agm, ["switch", String(profile.accountRef), "--target", "agy"], {
      encoding: "utf8", windowsHide: true, timeout: 20_000, stdio: ["ignore", "pipe", "pipe"], env: options.env || process.env,
    });
    return { ok: true };
  } catch (err) {
...
function launchRoutedCli(providerId, profile, options = {}) {
  if (!ROUTABLE_PROVIDERS.includes(providerId)) return { ok: false, reason: "unsupported-provider", providerId };
  const platform = options.platform || process.platform;
  if (platform !== "win32") return { ok: false, reason: "unsupported-platform", providerId };
  ...
  const executable = options.executable || resolveProviderExecutable(providerId, { env, platform, execFileSyncImpl: options.execFileSyncImpl });
  ...
  try {
    const child = spawnImpl(comspec, ["/d", "/s", "/c", commandLine], {
      cwd: options.cwd || process.cwd(), env, detached: true, windowsVerbatimArguments: true, windowsHide: false, stdio: "ignore",
    });
    if (child && typeof child.unref === "function") child.unref();
  } catch (err) {
    return { ok: false, reason: "spawn-failed", providerId, error: err.message || String(err) };
  }
  return { ok: true, providerId, executable, profileId: profile ? profile.id : null, accountLabel: profile ? profile.label : "기본 계정" };
}
...
function installRouterLaunchers(options = {}) {
  ...
  try {
    ...
    fs.writeFileSync(readme, [
      ...
      "Antigravity 다계정 전환은 agm의 암호화 계정 저장소를 사용합니다. 이메일/OAuth 토큰은 위젯 usage-cache에 저장하지 않습니다.",
...
module.exports = {
  PROVIDER_COMMANDS, PROVIDER_ARGS, PROVIDER_BIN_ENV, commandOutput, resolveProviderExecutable,
  cmdQuotedPath, safeArg, interactiveStartCommand, switchAntigravityProfile, applyDefaultEnvironment,
  launchRoutedCli, psLiteral, powershellRouterScript, cmdLauncherScript, installRouterLaunchers,
};
