...
function secretStatusSafe(settings = {}, force = false) {
  ...
  if (!force && secretStatusCache && secretStatusCacheKey === key) return { ...secretStatusCache };
  try {
  ...
  return {
    ...secretStatusCache,
    openRouterProfileStatuses: secretStatusCache.openRouterProfileStatuses.map((item) => ({ ...item })),
  };
}
