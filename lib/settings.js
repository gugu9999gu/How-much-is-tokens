...
const RESET_PRESERVED_FIELDS = ["githubToken", "cursorCookie", "accountProfiles"];
...
function resetSettingsValues(current = {}, options = {}) {
  ...
  const next = { ...DEFAULTS, smartRouting: normalizeSmartRouting() };
...
function secretStatusSafe(force = false) {
  if (!force && secretStatusCache) return { ...secretStatusCache };
  try {
  ...
  return { ...secretStatusCache };
}

function withSecretStatus(settings, force = false) {
  return { ...settings, ...secretStatusSafe(force) };
}
...
function saveSettings(next) {
  ...
  return withSecretStatus(merged, hasSecretOperation);
}
