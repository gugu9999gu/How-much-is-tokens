...
const { normalizeMediaProviderProfiles } = require("./media-provider-profiles");
...
const RESET_PRESERVED_FIELDS = ["githubToken", "cursorCookie", "accountProfiles", "openRouterProfiles", "mediaProviderProfiles"];
...
function normalizeCodexAutoResetProfiles(value, accountProfiles) {
  const valid = new Set(normalizeAccountProfiles(accountProfiles)
    .filter((profile) => profile.providerId === "codex" && profile.enabled !== false)
...
function normalizeSettings(settings) {
  ...
  delete next.mediaProviderProfileStatuses;
  ...
  next.codexAutoResetProfiles = normalizeCodexAutoResetProfiles(next.codexAutoResetProfiles, next.accountProfiles);
  ...
  next.mediaProviderProfiles = normalizeMediaProviderProfiles(next.mediaProviderProfiles);
...
function resetSettingsValues(current = {}, options = {}) {
  ...
  if (preserveCredentials) {
    for (const field of RESET_PRESERVED_FIELDS) {
      ...
      } else if (field === "mediaProviderProfiles") {
        next.mediaProviderProfiles = normalizeMediaProviderProfiles(current.mediaProviderProfiles);
...
function statusCacheKey(settings = {}) {
  const openRouter = normalizeOpenRouterProfiles(settings.openRouterProfiles)
    .map((profile) => profile.id)
    .join("|");
  const media = normalizeMediaProviderProfiles(settings.mediaProviderProfiles)
    .map((profile) => `${profile.providerId}:${profile.id}:${profile.mode}`)
    .join("|");
  return `${openRouter}||${media}`;
}
...
function cloneMediaProviderProfileStatuses(statuses) {
  return (Array.isArray(statuses) ? statuses : []).map((item) => ({
    ...(item || {}),
    fields: item && item.fields && typeof item.fields === "object" ? { ...item.fields } : {},
  }));
}
...
function cloneSecretStatus(status) {
  return {
    ...
    mediaProviderProfileStatuses: cloneMediaProviderProfileStatuses(status && status.mediaProviderProfileStatuses),
  };
}
...
function secretStatusSafe(settings = {}, force = false) {
  ...
  try {
    const status = require("./secure-secrets").secretStatus(settings.openRouterProfiles, settings.mediaProviderProfiles);
    secretStatusCache = {
      ...
      mediaProviderProfileStatuses: cloneMediaProviderProfileStatuses(status.mediaProviderProfileStatuses),
    };
...
function saveSettings(next) {
  ...
  let profileStructureChanged = false;
  ...
  if (Object.prototype.hasOwnProperty.call(patch, "mediaProviderProfiles")) {
    try {
      require("./secure-secrets").pruneMediaProviderProfileSecrets(merged.mediaProviderProfiles);
    } catch {}
    profileStructureChanged = true;
  }
  if (profileStructureChanged) invalidateSecretStatusCache();

  fs.mkdirSync(path.dirname(settingsPath()), { recursive: true });
  writeJson(settingsPath(), merged);
  return withSecretStatus(merged, hasSecretOperation || profileStructureChanged);
}
...
module.exports = {
  ...
  normalizeCodexAutoResetProfiles,
  normalizeSettings,
  resetSettingsValues,
  loadSettings,
  saveSettings,
