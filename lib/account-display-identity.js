...
function normalizeAccountIdentity(raw = {}) {
  ...
  // A provider usage row also has an ordinary `id` such as "codex". Never
  // interpret that generic field as an authenticated account identifier.
...
function claudeIdentity(profile = null) {
  ...
  return mergeAccountIdentity({
    ...
    accountId: oauth.accountUuid || oauth.accountUUID || oauth.userId || oauth.organizationUuid,
  }, identityFromClaims(claims));
}
...
function cursorIdentity(settings = {}) {
  try {
    ...
    return mergeAccountIdentity({ accountId: auth.userId }, identityFromClaims(decodeJwtPayload(auth.token)));
  } catch {
    return normalizeAccountIdentity();
  }
}
