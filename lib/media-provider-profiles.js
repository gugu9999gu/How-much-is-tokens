...
function officialMcpFor(providerId) {
  ...
  const entry = id && OFFICIAL_MCP[id];
  return entry ? { ...entry, balanceTools: [...entry.balanceTools] } : null;
}

function validMode(providerId, value) {
  return value === "mcp" && officialMcpFor(providerId) ? "mcp" : "api";
}
...
function normalizeMediaProviderProfiles(rawProfiles) {
  ...
  for (const raw of Array.isArray(rawProfiles) ? rawProfiles : []) {
    ...
    out.push({
      ...
      mode: validMode(providerId, raw.mode),
...
function nextMediaProfile(settings = {}, providerId, options = {}) {
  ...
  return {
    ...
    mode: validMode(id, options.mode),
    priority: ordinal * 10,
    enabled: true,
  };
}
