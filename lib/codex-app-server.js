...
const { preferredWindowsCommand } = require("./credential-login");
...
function nativeCodexExecutable(shimPath, platform = process.platform, arch = process.arch) {
  if (!shimPath || platform !== "win32") return null;
  const vendorArch = arch === "arm64" ? "arm64" : "x64";
...
function resolveCodexExecutable() {
  ...
  const preferred = process.platform === "win32" ? preferredWindowsCommand(found) : (found[0] || null);
  return nativeCodexExecutable(preferred) || preferred || null;
}
