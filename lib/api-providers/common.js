...
// Returns an empty object when the secure store is unavailable (e.g. the
// Electron-less CLI probe), so providers degrade to a "missing" state instead
// of throwing.
function loadApiCreds(providerId, fields) {
  try {
    return require("../secure-secrets").loadApiProviderSecrets(providerId, fields) || {};
  } catch {
    return {};
  }
}
...
function missingResult(def) {
  return {
    id: def.id,
    name: def.name,
    brand: def.brand,
    status: "missing",
    hint: def.missingHint || `설정 > 생성형 미디어 API에서 ${def.name} 자격증명을 저장하세요.`,
  };
}

function loginResult(def, error) {
  return {
    id: def.id,
    name: def.name,
    brand: def.brand,
    status: "login",
    error: error || null,
    hint: `${def.name} 인증이 거부되었습니다. 설정에서 API 키를 다시 저장하세요.`,
  };
}
...
async function runApiProvider(def, { url, headers, parse }) {
  const creds = trimmedCreds(def, loadApiCreds(def.id, credFields(def)));
  if (!hasAllCreds(def, creds)) return missingResult(def);
  ...
  if (res.status === 401 || res.status === 403) return loginResult(def, `HTTP ${res.status}`);
