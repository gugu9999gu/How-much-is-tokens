...
// GET /v1/creations/recent -> { data: [ { name, tool_name, created_at,
//   creation: { tool, status, family } } ], meta }
// Magnific/Freepik does not expose a remaining-credit endpoint publicly, so we
// surface the most recent generations as a usage log instead.
function parse(json) {
  ...
  const logs = items.slice(0, 5).map((item) => {
    ...
    const sub = [statusKo(creation.status), formatClock(parseTime(item.created_at))]
      .filter(Boolean)
      .join(" · ");
    return { label, sub };
  });
