...
async function connectOpenRouterOAuth(options = {}) {
  ...
  try {
    ...
    // OpenRouter OAuth can issue a different API key for the same user. For
    // +계정 only, compare the official current-key creator_user_id before
    // persisting the new profile. Existing multi-key profiles are left intact
    // because the local request router can intentionally use multiple keys.
