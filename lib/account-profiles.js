...
function defaultConfigDir(id) {
  ...
  if (normalized === "grok") return canonicalConfigDir(process.env.GROK_CONFIG_DIR || path.join(home(), ".grok"));
  return null;
}
...
function profileEnvironment(profile) {
  ...
  if (profile.providerId === "grok") return { ...process.env, GROK_CONFIG_DIR: profile.configDir };
  return process.env;
}
