...
async function inspectCli(providerId, options = {}) {
  ...
  if (!executable) {
    return {
      ...
      label: spec.label,
      installed: false,
      updateSupported: Array.isArray(spec.updateArgs),
...
async function updateCli(providerId) {
  const spec = resolvedSpec(providerId);
  if (!spec || !Array.isArray(spec.updateArgs)) {
    return { ok: false, providerId: String(providerId || ""), reason: "update-not-supported" };
  }
  const executable = resolveCommand(spec.commands);
  if (!executable) return { ok: false, providerId: spec.id, reason: "cli-not-found" };

  const result = await runResolvedCommand(executable, spec.updateArgs, { timeout: UPDATE_TIMEOUT_MS });
  if (!result.ok) {
    return {
      ok: false,
      providerId: spec.id,
