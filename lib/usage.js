...
function reconcileDuplicateManagedAccounts(providers) {
  try {
    ...
    if (reconciliation.duplicates.length) {
      saveSettings({
        accountProfiles: reconciliation.accountProfiles,
        openRouterProfiles: reconciliation.openRouterProfiles,
      });
    }
...
async function fetchAll(settings = {}, secrets = {}) {
  ...
  const fetchedProviders = sortProviders(await Promise.all(jobs));
