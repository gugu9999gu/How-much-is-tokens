...
function decorateProvider(provider, result) {
  return { ...result, ...providerMeta(provider.id) };
}
...
function reconcileDuplicateManagedAccounts(providers) {
  try {
    ...
    if (reconciliation.duplicates.length) saveSettings({ accountProfiles: reconciliation.accountProfiles });
    return reconciliation;
  } catch {
    return { providers: [...providers], duplicates: [] };
  }
}
...
async function fetchAll(settings = {}, secrets = {}) {
  ...
  for (const provider of PROVIDERS) {
    if (provider.id === "antigravity") {
      jobs.push(antigravityRows(settings, secrets));
      continue;
    }

    if (provider.id === "openrouter") {
      const apiProfiles = activeOpenRouterProfiles(settings);
      if (apiProfiles.length) apiProfiles.forEach((profile, index) => jobs.push(safeFetch(provider, settings, secrets, profile, null, index)));
      else jobs.push(safeFetch(provider, settings, secrets));
      continue;
    }
    ...
    if (API_PROVIDERS.includes(provider)) {
      ...
      jobs.push(safeFetch(provider, settings, secrets, null, null, 0));
      const mediaProfiles = activeMediaProviderProfiles(settings, provider.id);
      mediaProfiles.forEach((profile, index) => jobs.push(safeFetchMedia(provider, settings, secrets, profile, index + 1)));
      continue;
    }

    const profiles = byProvider.get(provider.id) || [];
    if (!defaultProviderDisabled(settings, provider.id)) jobs.push(safeFetch(provider, settings, secrets, null, profiles.length ? "기본 계정" : null, 0));
    profiles.forEach((profile, index) => jobs.push(safeFetch(provider, settings, secrets, profile, null, index + 1)));
  }
