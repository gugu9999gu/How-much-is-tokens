...
async function antigravityRows(settings, secrets) {
  try {
    ...
    if (Array.isArray(managed) && managed.length) {
      return managed.map((row, index) => decorateProvider(antigravity, {
        ...row,
        accountOrder: index,
        routingRemainingPct: derivedRoutingRemainingPct(row),
      }));
    }
  } catch {}
...
function reconcileDuplicateManagedAccounts(providers) {
  try {
    ...
    if (reconciliation.duplicates.length) {
      saveSettings({ accountProfiles: reconciliation.accountProfiles });
    }
