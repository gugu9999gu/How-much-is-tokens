...
assert.strictEqual(providerMeta("openrouter").vendor, "API 공급자");
assert.strictEqual(providerMeta("unknown").vendor, "기타");

const ids = ["openrouter", "copilot", "grokbot", "cursor", "claude", "antigravity", "grok", "codex", "unknown"];
const sorted = sortProviders(ids.map((id) => ({ id, name: id, ...providerMeta(id) })));
assert.deepStrictEqual(
  sorted.map((provider) => provider.id),
  ["codex", "claude", "antigravity", "grok", "grokbot", "cursor", "copilot", "openrouter", "unknown"],
);
